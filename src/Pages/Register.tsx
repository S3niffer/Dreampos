...
const Register = () => {
    ...
    const [RegisterParameters, setRegisterParameters] = useState<I_UserRegisterParameters>({
        Email: "",
        Name: "",
        Password: "",
    })
    ...
    const _FormChangerHandler = (e: React.FormEvent<HTMLFormElement>) => {
        if (
            (e.target as HTMLFormElement).form.elements[1].value === "" ||
            (e.target as HTMLFormElement).form.elements[2].value.length < 8 ||
    ...
    const _RegisterHandler = (e: React.FormEvent<HTMLFormElement>) => {
        e.preventDefault()
        ...
        <>
            <div className='login flex'>
                <div className='flex-1 bg-added-bg-primary p-6 pt-9 pb-2 min-h-screen'>
                    ...
                    <div className='mt-8 text-2xl'>ثبت نام</div>
                    <span className='block mt-6 text-added-text-secondary text-justify text-xs sm:text-sm lg:text-base'>
                        بدلیل امنیت پایین دیتابیس این پروژه لطفا از وارد کردن رمز یا حتی ایمیل های واقعی (که ممکنه دوست نداشته
                        باشید دست فرد غریبه بیوفته) جلوگیری کنید
                    </span>
                    <form
                        onSubmit={_RegisterHandler}
                    ...
                    >
                        <InputTag
                            ...
                            maxLength={30}
                            onChange={e => {
                                setRegisterParameters((prv): I_UserRegisterParameters => {
                                    return {
                                        ...prv,
                                        Name: e.target.value,
                                    }
                        ...
                        <InputTag
                            label='رمزعبور'
                            name='passowrd'
                            type={passowrdvisibility ? "text" : "password"}
                            Icon={passowrdvisibility ? FiEye : FiEyeOff}
                            placeholder='رمزعبور خودتون را وارد کنید'
                            max={16}
                            minLengthCondition={8}
                            FunctionOnIcon={() => setpassowrdvisibility(prv => !prv)}
                            onChange={e => {
                                setRegisterParameters((prv): I_UserRegisterParameters => {
                                    return {
                                        ...prv,
                            ...
                            value={RegisterParameters.Password}
                        />
                        <button
                            type='submit'
                    ...
                    <div className='relative p-4 w-full max-w-md max-h-full'>
                        <div className='relative bg-added-bg-primary rounded-lg shadow'>
                            ...
                            <div className='p-4 md:p-5  text-center'>
                                ...
                                <h3 className='text-lg font-normal text-added-text-primary mt-3 dir-rtl text-center'>
